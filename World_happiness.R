...
numerical <- c("life_evaluation",'log_gdp_per_capita', 'social_support','healthy_life_expectancy',"generosity", 'freedom', "corruption", "gov_effectiveness", "no_violence", "regulatory_quality", "rule_law", "voice_accountability", "positive_emotions", "negative_emotions", "donated", "volunteered", "helped_a_stranger", "meals_shared", "household_size")
ordered_vars <- c("life_evaluation", setdiff(numerical, "life_evaluation"))
whr_corr <- whr[ordered_vars]
...
ggcorrplot(
  ...
  tl.cex = 7,      # smaller text to avoid overlap
  tl.srt = 45
)
...
for (var in ranking_var) {
  whr[[var]] <- whr[[paste0(var, "_pct")]]  # overwrites the original column
...
res_base = resample(task_whr, baseline, cv20)
perf_base = res_base$aggregate(measures)

res_lm = resample(task_whr, lm_model, cv20)
perf_lm = res_lm$aggregate(measures)

res_tree = resample(task_whr, tree_model, cv20)
perf_tree = res_tree$aggregate(measures)

res_rf = resample(task_whr, rf_model, cv20)
...
#let's plot the decision tree to see the splits. For interpretability, we visualize a single regression tree trained on the training split, while model comparisonrelies on cross-validated performance.

# Apply hierarchical imputation to full task
task_imp <- po_hier$train(list(task_whr))[[1]]

# Extract data
whr_imp <- as.data.frame(task_imp$data())
set.seed(123)
n <- nrow(whr_imp)
train_idx <- sample(seq_len(n), size = 0.75 * n)

train_data <- whr_imp[train_idx, ]
test_data  <- whr_imp[-train_idx, ]
tree_rpart <- rpart(
  life_evaluation ~ .,
  data = train_data,
  method = "anova",
...
rpart.plot(
  ...
  fallen.leaves = TRUE,
  box.palette = "Blues",
  main = "Decision Tree (Train–Test Split)"
)
...
## 8.1 Data Preparation with Standardization ----

cat("\n--- Preparing Standardized Data ---\n")

# Create dataset for importance analysis
whr_for_importance <- whr[, c("life_evaluation", features)]

# Create task
task_importance = as_task_regr(
  whr_for_importance,
  target = "life_evaluation",
  id = "whr_importance"
)
...
whr_imputed <- po_hier$train(list(task_importance))[[1]]$data()
whr_imputed <- as.data.frame(whr_imputed)

# Standardize all features (excluding target variable)
feature_cols <- setdiff(names(whr_imputed), "life_evaluation")
whr_standardized <- whr_imputed
whr_standardized[, feature_cols] <- scale(whr_imputed[, feature_cols])

cat("Features standardized (mean = 0, sd = 1)\n")

## 8.2 Linear Regression: Coefficient Analysis ----

cat("\n--- Linear Regression Coefficients (Standardized) ---\n")

# Train linear model on standardized data
lm_std <- lm(life_evaluation ~ ., data = whr_standardized)

lm_coefs <- coef(lm_std)
lm_df <- data.frame(
...
lm_plot <- ggplot(lm_df[lm_df$Feature != "(Intercept)", ], 
  ...
  coord_flip() +
  labs(title = "Linear Regression Coefficients (Standardized Features)",
       x = "Feature",
       y = "Standardized Coefficient (Beta)") +
  theme_minimal()

print(lm_plot)

## 8.3 Random Forest: Permutation Importance ----

cat("\n--- Random Forest Permutation Importance (Standardized) ---\n")

library(iml)
library(ranger)

# Prepare data for Random Forest
y <- whr_standardized$life_evaluation
X <- whr_standardized[, -which(names(whr_standardized) == "life_evaluation")]

# Train Random Forest on standardized data
rf_model_iml <- ranger(
  life_evaluation ~ .,
  data = whr_standardized,
  num.trees = 500,
...
rf_imp_plot <- importance_rf$plot() +
  labs(title = "Random Forest Permutation Importance (Standardized Features)") +
  theme_minimal()

print(rf_imp_plot)

cat("\n--- Ranger Built-in Variable Importance (Standardized) ---\n")
ranger_imp <- sort(rf_model_iml$variable.importance, decreasing = TRUE)
print(ranger_imp)

# 9 Model Performances ----
##  9.1 Model Performance: MSE Fold Performance Boxplots ----
get_fold_mse <- function(resample_result) {
  sapply(resample_result$predictions(), function(pred) pred$score(msr("regr.mse")))
}

# Extract MSE per fold
perf_base_folds  <- get_fold_mse(res_base)
perf_lm_folds    <- get_fold_mse(res_lm)
perf_tree_folds  <- get_fold_mse(res_tree)
perf_rf_folds    <- get_fold_mse(res_rf)

perf_df <- data.frame(
  Model = rep(c("Baseline", "Linear Regression", "Decision Tree", "Random Forest"),
              each = length(perf_base_folds)),
  MSE = c(perf_base_folds, perf_lm_folds, perf_tree_folds, perf_rf_folds)
)

ggplot(perf_df, aes(x = Model, y = MSE, fill = Model)) +
...
##  9.2 Model Performance: R2 Fold Performance Boxplots ----
get_fold_r2 <- function(resample_result) {
  sapply(resample_result$predictions(), function(pred) pred$score(msr("regr.rsq")))
}

# Extract R2 per fold
perf_base_r2  <- get_fold_r2(res_base)
perf_lm_r2    <- get_fold_r2(res_lm)
perf_tree_r2  <- get_fold_r2(res_tree)
perf_rf_r2    <- get_fold_r2(res_rf)

r2_df <- data.frame(
  Model = rep(c("Baseline", "Linear Regression", "Decision Tree", "Random Forest"),
              each = length(perf_base_r2)),   # usually 10 folds
  R2 = c(perf_base_r2, perf_lm_r2, perf_tree_r2, perf_rf_r2)
)
...
ggplot(r2_df, aes(x = Model, y = R2, fill = Model)) +
  ...
  theme_minimal() +
  labs(
    title = "R² Performance Across CV Folds",
    y = "R² Score",
    x = ""
  ) +
  theme(
